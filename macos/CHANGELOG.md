...
- 全新安装时若活动主题目录尚不存在，先从内置主题安全初始化且不覆盖已有用户主题
- 修正内置主题引用了未随仓库发布的背景文件，恢复使用 bundled abstract demo 素材

### 视觉

- 默认主题更新为原创 APEX 53 赛车版画，使用红、浅蓝、白与深蓝配色
- Scheduled 搜索与状态筛选合并为深蓝 pit-wall 控制面板，补齐焦点态、选中态及暂停任务的有效对比度
- 完成全 App 对比度巡检：修正 Plugins 深色图文、浅色壳顶部工具、Composer 控件与侧栏 hover，并恢复首页建议卡显示
- APEX 53 固定使用深色 shell，不再跟随 Codex 浅色外观；输入框、占位文字与 Composer 控件改为深色背景上的高对比文字
